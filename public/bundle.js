...
/***/ (function(module, exports, __webpack_require__) {
	...
	var makeOutlineBolderIndicator = function makeOutlineBolderIndicator(state) {
	    return function (d, i) {
	        ...
	        if (state) {
	            ...
	            d3.selectAll('.indicator-group > path').filter(function (d) {
	                if (d.index === i) {
	                    return true;
	                } else {
	                    return false;
	                }
	            }).transition().style('fill', '#eaeaea').style('stroke-width', '2');
	        } else {
	            d3.selectAll('.indicator-group > path').filter(function (d) {
	                if (d.index === i) {
	                    return true;
	            ...
	            }).transition().style('stroke-width', '1').style('fill', '#fff');
	        }
	    };
	};
	
	var makeOutlineBolderComponent = function makeOutlineBolderComponent(state) {
	    return function (d, i) {
	
	        if (state) {
	
	            d3.selectAll('.component-group-main > path').filter(function (d) {
	                if (d.index === i) {
	                    return true;
	                } else {
	                    return false;
	            ...
	            }).transition().style('fill', '#fff').style('stroke-width', '1');
	        }
	    };
	};
	
	var makeOutlineBolderElement = function makeOutlineBolderElement(state) {
	    return function (d, i) {
	
	        if (state) {
	
	            d3.selectAll('.element-group-main > path').filter(function (d) {
	                if (d.index === i) {
	                    return true;
	                } else {
	                    return false;
	                }
	            }).transition().style('fill-opacity', '1').style('font-weight', 'bolder');
	        } else {
	            d3.selectAll('.element-group-main > path').filter(function (d) {
	                if (d.index === i) {
	                    return true;
	...
	var elementArc = d3.svg.arc().innerRadius(outerRadius - 30).outerRadius(outerRadius);
	...
	var componentFill = d3.scale.category10();
	var elementFill = d3.scale.category10();
	...
	svg.each(function (matrix, j) {
	    ...
	    indicatorGroups = svg.selectAll('g.indicator-group').data(indicatorLayout.groups).enter().append('svg:g').attr('class', 'indicator-group').on('mouseover', fade(0.1)).on('mouseout', fade(1)).on('click', function (d, i) {
	    ...
	    componentGroups = svg.selectAll('g.component-group-main').data(componentLayout.groups).enter().append('svg:g').attr('class', 'component-group-main').on('mouseover', showCorrespondingIndicatorChordsComponent(0.1)).on('mouseout', showCorrespondingIndicatorChordsComponent(1)).on('click', function (d, i) {
	        $('main .content-holder').css({
	            'display': 'block',
	            'flex-basis': '25%'
	        });
	    ...
	    }).attr('d', componentArc).append('svg:title').text(function (d, i) {
	    ...
	    // add element groups
	    elementGroups = svg.selectAll('g.element-group-main').data(elementLayout.groups).enter().append('svg:g').attr('class', 'element-group-main').on('mouseover', showCorrespondingIndicatorChordsElement(0.1)).on('mouseout', showCorrespondingIndicatorChordsElement(1)).on('click', function (d, i) {
	        $('main .content-holder').css({
	            'display': 'block',
	            'flex-basis': '25%'
	        });
	    ...
	    }).on('mouseover', makeOutlineBolderElement(1)).on('mouseout', makeOutlineBolderElement(0)).attr('id', function (d, i) {
	...
	var appendElementTemplate = function appendElementTemplate() {
	    $('.template').remove();
	    var html = '<div class="template">' + '<div class="headerColor"></div>' + '<h2 class="elementTitle"></h2>' + '<h5 class="elementTag"></h5>' + '<p class="elementBody"></p>' + '<div class="headerColor"></div>' + '</div>';
	    $('main .content-holder').append(html);
	};
	
	var appendComponentTemplate = function appendComponentTemplate() {
	    $('.template').remove();
	    var html = '<div class="template">' + '<h2 class="componentTitle"></h2>' + '<p class="componentBody"></p>' + '</div>';
	    $('main .content-holder').append(html);
	};
	
	var appendIndicatorTemplate = function appendIndicatorTemplate() {
	    $('.template').remove();
	    var html = '<div class="template">' + '<h2 class="indicatorTitle"></h2>' + '<h3 class="indicatorLevelsTitle">LEVELS</h3>' + '<h4>' + '<span class="levelOne"> </span>' + '</h4>' + '<p class="levelOneBody"> </p>' + '<h4>' + '<span class="levelTwo"> </span>' + '</h4>' + '<p class="levelTwoBody"> </p>' + '<h4>' + '<span class="levelThree"> </span>' + '</h4>' + '<p class="levelThreeBody"> </p>' + '<h4>' + '<span class="levelFour"> </span>' + '</h4>' + '<p class="levelFourBody"> </p>' + '</div>';
	    $('main .content-holder').append(html);
	};
